...
public class Utilidades {
     ...
     * @return 
     */
    public Object esJSON(String cadena) {
        
        try {
            return new JSONObject(cadena);
        } catch(JSONException e) {
            try {
                return new JSONArray(cadena);
        ...
        if (tipoLista.equals("simple")) {                        
            ...
            for (Row fila : ResultSet) {
                for (int i = 0; i < columnas.size(); i++) {
                    resultadoTransaccionArray.put(columnas.get(i).getType().deserialize(fila.getBytesUnsafe(columnas.get(i).getName())));
                }
            }                        
            ...
            for (Row fila : ResultSet) {
                for (int i = 0; i < columnas.size(); i++) {
                    resultadoTransaccionObject.put(columnas.get(i).getName(), 
                                                   columnas.get(i).getType().deserialize(fila.getBytesUnsafe(columnas.get(i).getName())));                                
                }
                resultadoTransaccionArray.put(resultadoTransaccionObject);
            ...
            if (tipoLista.equals("parValoresSimple")) {
                ...
                for (Row fila : ResultSet) {
                    resultadoTransaccionObject.put(columnas.get(0).getType().deserialize(fila.getBytesUnsafe(columnas.get(0).getName())).toString(), 
                                                   columnas.get(1).getType().deserialize(fila.getBytesUnsafe(columnas.get(1).getName())));
            ...
            if (tipoLista.equals("parValoresCompuesta")) {
                ...
                JSONObject grupoActual;
                for (Row fila : ResultSet) {
                    grupoActual = new JSONObject();
                    for (int i = 1; i < columnas.size(); i++) {
                        grupoActual.put(columnas.get(i).getName(), 
                                    columnas.get(i).getType().deserialize(fila.getBytesUnsafe(columnas.get(i).getName())));                                
                    }
                    resultadoTransaccionObject.put(columnas.get(0).getType().deserialize(fila.getBytesUnsafe(columnas.get(0).getName())).toString(),
                                                   grupoActual);
    ...
    public Object transformarParametro (String nombreTransformacion, Object parametro) throws Exception {
        ...
        switch(Constantes.valueOf(nombreTransformacion)) {
            ...
            case T_EMAIL:        
                ...
                transformacion.put("usuario", StringUtils.substringBefore(parametro.toString(), "@"));
                transformacion.put("dominio", StringUtils.substringAfter(parametro.toString(), "@"));
